...
## Recipe 7：知识库智能问答（聚合远程知识库 MCP）

适用：用户问"X 流程是什么"、"操作手册在哪"、"系统支持 Y 吗"等需要从企业飞书 Wiki 等知识库取答案的场景。

```
1. 直接调用 executeApi("askKnowledgeBase", { question: "<原问题>" })
   - 不必先 listAllApis，本 API 是"高频且自包含"的
   - 服务端内部子 agent 会自动连接知识库 MCP，迭代搜索→读取→汇总
2. 响应字段：
   - answer：最终答案（可直接呈给用户）
   - finishReason：answered / max_steps / timeout / loop_guard / no_kb
   - durationMs / steps / trace：观测信息，必要时摘要给用户（不要全 dump）
3. 处置 finishReason：
   - answered → 直接采用 answer
   - max_steps → 可建议用户拆分问题或调大 maxSteps
   - timeout → 调大 timeoutMs，或缩小 question 范围后重试
   - no_kb → 知识库未就绪，告知用户并停止
   - loop_guard → 换关键词重试一次
4. 触发 nextHint 时按其建议执行；连续两次失败即停下问用户
```

**省 token 守则**：
- 子 agent 已在服务端做"内层多轮"，**外层不要再就同一 question 反复调用**
- 同一会话已得到 answer 的问题，不必再次调用——直接复用
- 默认带 10 分钟 LRU 缓存，相同问题的二次调用响应里会带 `fromCache: true`

**与业务 API 联动**：
- 先用 `askKnowledgeBase` 了解某流程概念 → 再用业务 API 实际执行
- 例：用户问"如何创建外汇任务" → 先问知识库拿步骤说明 → 再调对应业务 API 创建
